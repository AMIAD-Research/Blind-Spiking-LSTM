...
def make_input(config, sk, batch, seed=0):
    ...
    flat = vmap(encrypt, (0, None, None, 0))(jnp.round(beta_x * x_poly),
                                             sk, dict_params, keys)
    # (B*L, ...) -> (B, L, ...)
    return jax.tree.map(lambda a: a.reshape((batch, L) + a.shape[1:]), flat)
...
# --------------------------------------------------------------------------- #
#  Sous-ops, vmappees sur l'axe de batch de tete (modele/LUT partages)
# --------------------------------------------------------------------------- #
@partial(jax.jit, static_argnames=["model"])
def PBS_sigmoid(model, input_boot):
    return vmap(lambda z: model.bootstrapping(z, model.LUT_sigmoid))(input_boot)


@partial(jax.jit, static_argnames=["model"])
def PBS_tanh(model, input_boot):
    return vmap(lambda z: model.bootstrapping(z, model.LUT_tanh))(input_boot)
...
def PBS_square_minus(model, x, y):
    def one(x, y):
        ...
        return model.bootstrapping(minus, model.LUT_square)
    return vmap(one)(x, y)


@partial(jax.jit, static_argnames=["model"])
def packing(model, input_packing):
    return vmap(model.heavyBS.packing)(input_packing)


def reshape_lut(model, obj):
    """(B, hidden*degree_lut) -> (B, hidden, degree_lut), idem pour le body."""
    B = obj[0].shape[0]
    return (obj[0].reshape(B, model.hidden_dim, model.degree_lut),
            obj[1].reshape(B, model.hidden_dim, 1))
...
def main():
    ...
    print("== setup LSTM ==")
    print(f"  hidden_dim={model.hidden_dim}  batch={B}")

    X_cipher = make_input(config, sk, B, args.seed)
    x_t = jax.tree.map(lambda a: a[:, 0], X_cipher)          # (B, ...)
    H, C = warm_carry(model, X_cipher, config, args.warm_step)
    dim = jnp.arange(model.hidden_dim)
    ### Micro benchmark
    #
    # breakpoint()
    C_lwe = vmap(vmap(sample_extract, (None, 0)), (0, None))(C, dim)

    ### first forward
    # Linear
    HX = vmap(lambda a, b: sum_ciphertext_ciphertext(a, b, dict_params["q"]))(x_t, H)
    F_t = vmap(model.W_f)(HX)
    I_t = vmap(model.W_i)(HX)
    C_t = vmap(model.W_c)(HX)
    O_t = vmap(model.W_o)(HX)
    ...
    f_c = [
        centered_mod(f_plus_c[0] - f_minus_c[0], dict_params["q"]).reshape(B, model.hidden_dim, model.degree_lut),
        centered_mod(f_plus_c[1] - f_minus_c[1], dict_params["q"]).reshape(B, model.hidden_dim, 1),
    ]

    # Input
    i_time = bench(PBS_sigmoid, model, I_t, reps=args.reps, warmup=args.warmup)
    i_obj = reshape_lut(model, PBS_sigmoid(model, I_t))

    # Candidate
    c_time = bench(PBS_tanh, model, C_t, reps=args.reps, warmup=args.warmup)
    c_obj = reshape_lut(model, PBS_tanh(model, C_t))
    ...
    i_c = [
        centered_mod(i_plus_c[0] - i_minus_c[0], dict_params["q"]).reshape(B, model.hidden_dim, model.degree_lut),
        centered_mod(i_plus_c[1] - i_minus_c[1], dict_params["q"]).reshape(B, model.hidden_dim, 1),
    ]
    ...
    o_obj = reshape_lut(model, PBS_sigmoid(model, I_t))

    # Tanh C_out
    tanh_c_time = bench(PBS_tanh, model, C_out, reps=args.reps, warmup=args.warmup)
    tanh_c_obj = reshape_lut(model, PBS_tanh(model, C_out))
    ...
    print(f"f_t : {1000*f_time/B}")
    print(f"i_t : {1000*i_time/B}")
    print(f"c'_t : {1000*c_time/B}")
    print(f"o_t : {1000*o_time/B}")

    print(f"f_t . c_t_1 : {1000*(f_plus_c_time+f_minus_c_time)/B}")
    print(f"i_t . c_t : {1000*(i_plus_c_time+i_minus_c_time)/B}")
    print(f"tanh_c_t : {1000*tanh_c_time/B}")
    print(f"o_t .tanh_c_t :{1000*(tc_plus_o_time+tc_minus_o_time)/B}")

    print(f"Packing : {1000*packing_time/B}")
    print()
    PBS_tot_time = f_time + i_time + c_time + o_time + tanh_c_time
    multiplication_time = (f_plus_c_time + f_minus_c_time + i_plus_c_time
                            + i_minus_c_time + tc_plus_o_time + tc_minus_o_time)
    ...
    print(f"Total runtime for single timestep (batch={B}) : {jnp.round(1000*total_time_per_timestep)}ms")
    print(f"Total runtime per sample : {jnp.round(1000*total_time_per_timestep/B, 2)}ms")
    print(f"Cout d'un PBS : {1000*f_time/(model.hidden_dim*B)}ms")
