...
def _ffi_call(
    ...
    all_rot_fft: jax.Array,           # NOUVEAU: La table entière, sans batch
    all_rotations_indices: jax.Array, # NOUVEAU: Les indices batchés
    log_beta: int,
    log_q: int,
    l: int,
) -> jax.Array:
    ...
    call = jax.ffi.ffi_call(
        ...
        jax.ShapeDtypeStruct(
            (bsk_reshaped.shape[1], ct_init.shape[-1]), # CORRECTION : on supprime shape[:-1]
            jnp.int64
        ),
    ...
    result = call(
        ct_init,
        bsk_reshaped,
        all_rot_fft,             # Transmission de la table globale
        all_rotations_indices,   # Transmission des indices
        log2_beta=log_beta,
        log2_q=log_q,
        l=l,
...
def multiply_seq_monomial(
    ...
    # Les indices conservent leur dimension de batch grâce au vmap
    ...
    # SUPPRIMÉ : rot_fft = all_rot_fft[all_rotations_indices] -> Adieu le OOM !
