...
# --------------------------------------------------------------------------- #
#  Sous-ops, vmappees sur l'axe de batch de tete (LUT/cles partagees)
# --------------------------------------------------------------------------- #
@partial(jax.jit, static_argnames=["model"])
def LUT(model, C_lwe_reshape):
    def one(c):
        return vmap(vmap(model.heavyBS.prepare_LUT, (0, 0)), (0, 0))(c, model.lut_f)
    return vmap(one)(C_lwe_reshape)
...
def main():
    ...
    print("== setup BSLSTM ==")
    print(f"  hidden_dim={model.hidden_dim}  n_lut={model.n_lut}  batch={B}")
    ...
    C_lwe_reshape = (
        C_lwe[0].reshape(B, model.n_lut, model.hidden_dim // model.n_lut, model.degree_lut),
        C_lwe[1].reshape(B, model.n_lut, model.hidden_dim // model.n_lut, 1),
    )
    ...
    lut_C_t_1 = LUT(model, C_lwe_reshape)

    lut_C_t_1 = centered_mod_ciphertext(
        (jnp.sum(lut_C_t_1[0], axis=1), jnp.sum(lut_C_t_1[1], axis=1)), dict_params["q"]
    )
    ...
    LUT1_obj = centered_mod_ciphertext(
        (jnp.sum(LUT1_obj[0], axis=1), jnp.sum(LUT1_obj[1], axis=1)), model.dict_params["q"]
    )

    PBS_f_time = bench(PBS, model, F_t, LUT1_obj, reps=args.reps, warmup=args.warmup)
    F = PBS(model, F_t, LUT1_obj)
    F = (
        F[0].reshape(B, model.hidden_dim, model.degree_lut),
        F[1].reshape(B, model.hidden_dim, 1),
    )

    # Input
    PBS_i_time = bench(PBS_shared_lut, model, F_t, model.lut_i, reps=args.reps, warmup=args.warmup)
    I = PBS(model, F_t, LUT1_obj)
    C_out_lwe = (
        centered_mod(I[0].reshape(B, model.hidden_dim, model.degree_lut) + F[0], dict_params["q"]),
        centered_mod(I[1].reshape(B, model.hidden_dim, 1) + F[1], dict_params["q"]),
    )

    C_out_lwe_reshape = (
        C_out_lwe[0].reshape(B, model.n_lut, model.hidden_dim // model.n_lut, model.degree_lut),
        C_out_lwe[1].reshape(B, model.n_lut, model.hidden_dim // model.n_lut, 1),
    )
    I = (
        I[0].reshape(B, model.hidden_dim, model.degree_lut),
        I[1].reshape(B, model.hidden_dim, 1),
    )
    ...
    print(f"f_t : {1000*PBS_f_time/B}")
    print(f"i_t : {1000*PBS_i_time/B}")
    print(f"o_t : {1000*PBS_o_time/B}")

    print(f"LUT setup f_t : {1000*LUT_F_time/B}")
    print(f"LUT setup i_t : {1000*LUT_O_time/B}")
    ...
    print(f"Cout d'un PBS : {1000*PBS_f_time*model.n_lut/(model.hidden_dim*B)}ms")
